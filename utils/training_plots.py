...
"""Generate plots and model evaluation metrics from training"""
...
def saveConfig(path, args):
    with open(os.path.join(path, "train_configs.txt"), 'w', encoding="utf-8") as f:
...
def drawFig(history, examine, monitor, savingPath):
    ...
    with open(os.path.join(savingPath, "train_result.txt"), 'w') as f:
    ...
    plt.xlabel('epoch')
    plt.savefig(os.path.join(savingPath, "train_result.jpg"))